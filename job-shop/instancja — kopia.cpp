...
void instancja::update(int start, vector<zad> &oZad)	// weryfikacja i update
{
	...
	for (int i = 0; i <= start; i++)
		if (rozwiazanie[1][start].zadanie == rozwiazanie[2][i].zadanie) // szukamy op1 zadania
		{
			findtask = i;
			break;
		}

	findtaskif = findtask < 0 ? false : true; // znalezlismy cos ?
	if (!findtaskif)//nie to ustaiwamy findtask na 0 by uniknac VECTOR OUT OF RANGE !@#
		findtask = 0;

	if (czas[1][start].begin < oZad[findtask].czas_konca && rozwiazanie[1][start].operacja == 2 && findtaskif)//sprawdzamy czy op2 nie wykonuje sie przed op1
	{
		this->czas[1][start].begin = oZad[findtask].czas_konca;
		this->czas[1][start].end = czas[1][start].begin + czas[1][start].time;
		for (int j = start + 1; j < ile; j++)//uaktalniamy czas maszyny 1
		{
